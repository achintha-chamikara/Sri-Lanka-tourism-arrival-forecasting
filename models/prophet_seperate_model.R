# prophet_seperate_model.R (Regime Prophet) - fixed + improved
# - add_seasonality() used BEFORE fitting (fit=FALSE + fit.prophet)
# - Predict per-regime to keep vector lengths aligned
# - Returns BOTH full + meaningful metrics (option)
# NOTE: This evaluates in-sample unless prophet_df is already your test window.
# For a fair comparison, pass only your TEST portion to this function (or refactor to train/test).
...
run_regime_prophet <- function(prophet_df,
  ...
  if (add_monthly_seasonality) {
    m1 <- add_seasonality(m1, name = "monthly", period = 30.5, fourier.order = 5)
  }
  ...
  if (length(idx1) > 0) {
    fc1 <- predict(m1, data.frame(ds = df_pred$ds[idx1]))
    df_pred$predicted[idx1] <- as.numeric(fc1$yhat)
  }

  idx2 <- which(df_pred$regime == "easter_covid")
  if (length(idx2) > 0) {
    fc2 <- predict(m2, data.frame(ds = df_pred$ds[idx2]))
    df_pred$predicted[idx2] <- as.numeric(fc2$yhat)
  }

  idx3 <- which(df_pred$regime == "post_covid")
  if (length(idx3) > 0) {
    fc3 <- predict(m3, data.frame(ds = df_pred$ds[idx3]))
    df_pred$predicted[idx3] <- as.numeric(fc3$yhat)
  }
